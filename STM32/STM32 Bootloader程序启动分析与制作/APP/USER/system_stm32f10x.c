...
static void SetSysClockTo72(void) {
    ...
    // RCC->CFGR = 0x00000000
    // RCC->CIR = 0x00000000
    RCC->CR |= ((uint32_t)RCC_CR_HSEON);  // RCC->CR = 0x00035683
    ...
    if (HSEStatus == (uint32_t)0x01) {
        ...
        /* PCLK1 = HCLK */
        RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE1_DIV2;  // RCC->CFGR = 0x00000400
        ...
        RCC->CFGR &= (uint32_t)((uint32_t)~(
            RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMULL));      // // RCC->CFGR = 0x00000400
        RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLMULL9);  // RCC->CFGR = 0x001D0400
    #endif /* STM32F10X_CL */

        /* Enable PLL */
        RCC->CR |= RCC_CR_PLLON;  // 0x03035683
        ...
        /* Select PLL as system clock source */
        RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));
        RCC->CFGR |= (uint32_t)RCC_CFGR_SW_PLL;  // RCC->CFGR = 0x001D040A
