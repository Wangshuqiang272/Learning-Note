...
​	本人的开发版为STM32F103ZET6，Flash大小为**512K**，SRAM大小为**64K**，从下方示意图可得出，SRAM地址空间为**0x20000000-0x20010000**。存储代码的Flash地址空间为**0x0800 0000-0x0807 FFFF**。
...
​	以上为STM32内存分布情况，低地址存放着已初始化的全局/静态变量(从flash中拷贝到RAM中)以及未初始化的全局/静态变量，紧接着存放的是堆区，栈区。

​	堆区：当我们调用malloc函数时，将从堆区分配空间给我们，返回的地址随着调用函数的次数而增大，由此得出堆区是向上增长的。

​	栈区：当出现函数调用，局部变量申请时，都将消耗栈区的空间资源，可以看到，MSP指针地址时逐渐减小的，当发生弹栈时，MSP指针又将逐渐增大。此时我们可以认为，栈区是向下增长的。

​	注意：上述说的栈，与数据结构中的栈不相同，上述说的是STM32的硬件栈，而数据结构中的栈指代的是软件栈。虽然二者都遵循先进后出的基本理念，但是。STM32硬件栈遵循**入栈时，MSP 减小（向低地址移动）**，**出栈时，MSP 增大（向高地址移动）**。而软件栈遵循**入栈，top增大**，**出栈，top减小**